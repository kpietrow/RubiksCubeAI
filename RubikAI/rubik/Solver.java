...
public class Solver {
	...
	private int[][] cornerAlignment = {{-1}, {-1}, {-1}, {-1}, {-1}, {-1}, {-1}, {-1}};
	...
	public void cornerParity() {
		...
		int[][] corners = {cornerAlignment[0], cornerAlignment[1], cornerAlignment[2], cornerAlignment[3]};
		int[] corner = null;
		int parity = 0;
		...
		for (int i = 0; i < givenCorners.length; i++) {
			...
			for (int x = 0; x < validCorners.length; x++) {
				if (Arrays.equals(corner, validCorners[x])) {
					...
					cornerAlignment[x] = givenCorners[i];
				}
			}
			
			if (!valid) {
				System.out.println("false combo");
				System.exit(0);
			}	
		}
	}
