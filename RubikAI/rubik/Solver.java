...
public class Solver {
	...
	public void permutationParity() {
		...
		for (int i = 0; i < givenEdgesPositions.length; i++) {
			edge = givenEdgesPositions[i];
			
			for (int x = i + 1; x < givenEdgesPositions.length; x++) {
				if (edge > givenEdgesPositions[x]) {
					edgeParity++;
				}
			}
		}
		
		for (int i = 0; i < givenCornersPositions.length; i++) {
			corner = givenCornersPositions[i];
			
			for (int x = i + 1; x < givenCornersPositions.length; x++) {
				if (corner > givenCornersPositions[x]) {
					cornerParity++;
				}
			}
		}
		
		if ((edgeParity + cornerParity) % 2 != 0) {
			System.out.println("false");
			System.exit(0);
		}
	}
